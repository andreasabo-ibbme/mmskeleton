...
def train(
    ...
    try:
        ...
        for rep_num in range(num_reps_for_var):
            # Remove the results files from the previous run if they exist
            final_results_base = work_dir
            final_results_path = os.path.join(final_results_base, 'all_final_eval', wandb_group)

            try:
                print('Now on rep: ', rep_num)
                print("cleaning up from previous run...")
                shutil.rmtree(final_results_path)
            except:
                if rep_num > 0:
                    logging.exception('failed to clean up previous folder')
        ...
        logging.exception("got this: ")    

    
    try:
        shutil.rmtree(work_dir)
    except:
        print('failed to delete the OVERALL folder')
...
def train_model(
    ...
    # [('train', 5), ('val', 1)]
    _, num_epochs = runner.run(data_loaders, workflow, total_epochs, loss=loss)
