...
def batch_processor(model, datas, train_mode, loss):
    ...
    # Change the zero values if needed:
    condition = y_true == 0
    row_cond = condition.all(1)
    y_true_for_loss = copy.deepcopy(y_true)
    y_true_for_loss[row_cond, :] = zero_value

    losses = loss(output, y_true_for_loss)

    if type(loss) == type(mse_loss):
        if balance_classes:
            losses = weighted_mse_loss(output, y_true_for_loss, class_weights_dict)
