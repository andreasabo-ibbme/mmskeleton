...
def train(
    ...
    listing = glob.glob(data_dir_all_data)
    ...
    for subdir in listing:
        ...
        walk_names = [f for f in os.listdir(subdir) if os.path.isfile(os.path.join(subdir, f))]
        all_file_names_only.extend(walk_names)
        subFilesList = [os.path.join(subdir, f) for f in walk_names]
        all_full_files.extend(subFilesList)


    print("all files: ", len(all_full_files))
    all_file_names_only = list(set((all_file_names_only)))
    # input('files')
    ...
    listing = glob.glob(data_dir_pd_data)


    pd_all_files  = []
    pd_all_file_names_only = []
    for subdir in listing:
        # print(os.listdir(subdir))
        walk_names = [f for f in os.listdir(subdir) if os.path.isfile(os.path.join(subdir, f))]
        pd_all_file_names_only.extend(walk_names)
        subFilesList = [os.path.join(subdir, f) for f in walk_names]
        pd_all_files.extend(subFilesList)
    ...
    for test_id in test_ids:
        try:
            ...
            # TODO: fix this to extract the necessary files from all folders
            test_walks_pd_labelled = extractFullWalkFromFolder(data_dir_pd_data, test_subj_walks_name_only_pd_only)
            non_test_walks_pd_labelled = extractFullWalkFromFolder(data_dir_pd_data, non_test_subj_walks_name_only_pd_only)
            non_test_walks_all = extractFullWalkFromFolder(data_dir_all_data, non_test_subj_walks_name_only_all)
            non_test_walks_all_no_pd_label = extractFullWalkFromFolder(data_dir_all_data, non_test_subj_walks_name_only_non_pd_only)
            ...
            print(f"{len(test_walks_pd_labelled)}, {len(non_test_walks_pd_labelled)}, {len(non_test_walks_all)}, {len(non_test_walks_all_no_pd_label)}")
            # input("stop")
            ...
            num_reps = 1
            for train_ids, val_ids in kf.split(non_test_subj_walks_name_only_non_pd_only):
                if num_reps > 1:
                    break
                ...
                unlabelled_train = [non_test_subj_walks_name_only_non_pd_only[i] for i in train_ids]
                unlabelled_val = [non_test_subj_walks_name_only_non_pd_only[i] for i in val_ids]

                # Now convert this back to full file names
                unlabelled_train = extractFullWalkFromFolder(data_dir_all_data, unlabelled_train)
                unlabelled_val = extractFullWalkFromFolder(data_dir_all_data, unlabelled_val)


                # These are from the pd labelled set
                num_reps_pd = 0
                for train_ids_pd, val_ids_pd in kf.split(non_test_subj_walks_name_only_pd_only):
                    ...
                    labelled_train = [non_test_subj_walks_name_only_pd_only[i] for i in train_ids_pd]
                    labelled_val = [non_test_subj_walks_name_only_pd_only[i] for i in val_ids_pd]

                    labelled_train = extractFullWalkFromFolder(data_dir_pd_data, labelled_train)
                    labelled_val = extractFullWalkFromFolder(data_dir_pd_data, labelled_val)
...
def extractFullWalkFromFolder(folders_regex, walks):
    folders = glob.glob(folders_regex)
    print(len(walks))
    all_fullfiles = []
    walk_set = set(walks)
    for folder in folders:
        available_walks = [f for f in os.listdir(folder) if os.path.isfile(os.path.join(folder, f))]
        available_walks_set = set(available_walks)
        walks_to_extract = available_walks_set.intersection(walk_set) # only extract walks that are available and of interest

        full_path_from_folder = [os.path.join(folder, f) for f in walks_to_extract]
        all_fullfiles.extend(full_path_from_folder)

    return sorted(all_fullfiles)
